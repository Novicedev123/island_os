          ...
          <div className="flex justify-between items-center mb-10">
            ...
            <div className="flex gap-2">
              {['Monthly', 'Quarterly', 'Yearly'].map(t => (
                <button key={t} className={`px-4 py-1.5 rounded-xl text-xs font-bold transition-all ${t === 'Monthly' ? 'forest-gradient text-white shadow-lg shadow-island-green/20' : 'bg-slate-50 text-slate-400 hover:text-slate-600'}`}>
                  {t}
                </button>
              ))}
            </div>
