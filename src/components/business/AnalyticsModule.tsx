        ...
        <div className="lg:col-span-2 bg-white p-10 rounded-[3rem] border border-slate-100 shadow-sm">
          <div className="flex justify-between items-center mb-10">
            ...
            <div className="flex gap-2">
              {['7D', '30D', '1Y'].map(t => (
                <button key={t} className={`px-4 py-1.5 rounded-xl text-xs font-bold transition-all ${t === '30D' ? 'forest-gradient text-white shadow-lg shadow-island-green/20' : 'bg-slate-50 text-slate-400 hover:text-slate-600'}`}>
                  {t}
                </button>
              ))}
            </div>
